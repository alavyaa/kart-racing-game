...
export class Room {
  ...
  addPlayer(playerId, username) {
    ...
    const spawnPoint =
      GAME_CONSTANTS.SPAWN_POINTS[
        this.players.size % GAME_CONSTANTS.SPAWN_POINTS.length
      ];

    // ✅ assign color
    const color =
      PLAYER_COLORS[this.colorIndex % PLAYER_COLORS.length];
  ...
  getState() {
    return {
      ...
      players: Array.from(this.players.values()).map((p) => p.getState()), // ✅ FIXED
